...
fn op_close(state: State, rid: u32, _buf: Option<ZeroCopyBuf>) -> Result<u32, Error> {
    debug!("close rid={}", rid);
...
fn op_listen(state: State, _rid: u32, _buf: Option<ZeroCopyBuf>) -> Result<u32, Error> {
    debug!("listen");
...
fn op_accept(
    ...
    poll_fn(move |cx| {
        ...
        listener
            .poll_accept(cx)
            .map_ok(|(stream, _addr)| resource_table.add("tcpStream", Box::new(stream)))
    })
}
